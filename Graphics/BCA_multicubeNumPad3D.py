...
class Window(tk.Frame):
    ...
    def __init__(self, master, CommPortID):
        ...
        self.canvas.get_tk_widget().grid(column=0, row=0, rowspan=2)

        ##### Control buttons #####
        ControlFrame = tk.Frame(master)
        ControlFrame.grid(column=1, row=1)

        # New setup for energy and mass selection
        self.master.bind("<Return>", self.runSimulation)
        self.inputString = tk.StringVar()
        self.inputEntry = tk.Entry(ControlFrame, textvariable=self.inputString, font=tkFont.Font(size=30))
        self.inputEntry.grid(column=1, row=3, padx=20)
        ...
        self.SetupLabel.grid(column=1, row=4, padx=20)

        # 3x3 grid for mass and energy combinations
        self.createCombinationGrid(ControlFrame)
    ...
    def createCombinationGrid(self, parent):
        energies = [5, 20, 40]
        masses = [1, 29, 118]
        for i in range(3):
            for j in range(3):
                label = tk.Label(parent, text=f"E: {energies[i]} KeV\nM: {masses[j]} amu", borderwidth=1, relief="solid")
                label.grid(row=i, column=j, padx=10, pady=10)

    def runSimulation(self, event=None):
        input_val = self.inputString.get().strip()
        if not input_val or len(input_val) < 2:
            print("Invalid input. Please enter a pattern followed by a number (e.g., *1, -2, +3, /4).")
            return

        pattern = input_val[0]
        try:
            number = int(input_val[1])
        except ValueError:
            print("Invalid input. The second character must be a number between 1 and 9.")
            return
        ...
        if number < 1 or number > 9:
            print("Invalid input. The number must be between 1 and 9.")
            return

        energy_idx = (number - 1) // 3
        mass_idx = (number - 1) % 3
        self.SpeedInput.set([5, 20, 40][energy_idx])
        self.MassInput.set([1, 29, 118][mass_idx])

        if pattern == '/':
            self.clearSim()
            self.injectGrid()
        else:
            self.fireSim(pattern)

    def generateSim(self, pattern):
        global dT, Elements
        ...
        if pattern == '*':
            positions = [SimConfig["Ion_Position"]]
        elif pattern == '-':
            positions = [SimConfig["Ion_Position"] + np.array([x, y, 0]) * 0.02 for x in [-1, 0, 1] for y in [-1, 0, 1]]
        elif pattern == '+':
            positions = [SimConfig["Ion_Position"] + np.array([x, y, 0]) * 0.02 for x, y in [(-1, 0), (1, 0), (0, -1), (0, 1), (0, 0)]]

        totalEnergy = 0
        for pos in positions:
            Ion = particle(pos, Vel, Mass, Col=[1, 1, 1])
            Ion.Trail = False
            Ion.TrailCol = [1, 0, 0]
            Ion.DrawPri = 2
    ...
    def fireSim(self, pattern):
        self.clearSim()
        self.generateSim(pattern)
    ...
    def injectGrid(self):
        global dT_grid
        positions = [(x, y) for x in np.linspace(0, 1, 3) for y in np.linspace(0, 1, 3)]
        Mass = self.MassInput.get()
        Mass = float(Elements[int(Mass)][3])
        Vel = np.sqrt(self.SpeedInput.get() * 100) / Mass * np.array([0, 0, -0.1])
        Vel = 0.3 * self.SpeedInput.get() * np.array([0, 0, -0.1])
        ...
        for pos in positions:
            ion_pos = np.array([pos[0], pos[1], SimConfig["Ion_Position"][2]])
            Ion = particle(ion_pos, Vel, Mass, Col=[1, 1, 1])
            Ion.Trail = False
            Ion.TrailCol = [1, 0, 0]
            Ion.DrawPri = 2
            self.Particles.append(Ion)
    ...
    def GetSetupInfo(self):
        ...
        E = self.SpeedInput.get()
        self.SetupLabel["text"] = self.InfoTemplate.format(round(E), round(N))
    ...
    def drawParticles(self):
        ...
        for P in self.Particles:
            Pos = np.copy(P.Pos)
            self.ax.scatter(Pos[0], Pos[1], Pos[2], color=self.convertColour(P.Col))
